...
pathname <- "~/Desktop/GurindjiKriol" #set working path, e.g. a folder called "BayesVarbrul" on desktop
setwd(pathname)
...
data <- NULL
factor.name <- c("HH1","HL2","HH2","LL2","LH3","LL3","HH3","AGE","GENDER") #the column names of the social factors in the rawdata that you want to include in the analysis #reference is High exposure Low education G1 (HL1) female
data$Xind <- t(rawdata[-1,factor.name])
...
for (ii in 1:L) {
	...
	for (j in 1:TT) {
		...
		m <- post$m[j]
		beta <- as.numeric(post[j,rownames(data$Xind)])
		a <- N/(N+sum(r_tmp))
		b <- r_tmp/(N+sum(r_tmp))
...
for (ii in 1:L) {
	...
	bf[ii] <- integrate(integrad,lower=0,upper=100,theta0=theta0.post[[ii]],theta=theta.post[[ii]],omega.post=omega.post,post=post,X=data$Xind,nl.i=nl[ii],anc=anc,K=K,TT=TT,factor.name=factor.name)$value
	bf.gen[ii] <- integrate(integrad.ind,lower=0,upper=100,theta0=theta0.post[[ii]],theta=theta.post[[ii]],omega.post=omega.post,post=post,X=data$Xind,nl.i=nl[ii],anc=anc,K=K,TT=TT,factor.name=factor.name,ind.name="gen")$value
	bf.edu[ii] <- integrate(integrad.ind,lower=0,upper=100,theta0=theta0.post[[ii]],theta=theta.post[[ii]],omega.post=omega.post,post=post,X=data$Xind,nl.i=nl[ii],anc=anc,K=K,TT=TT,factor.name=factor.name,ind.name="edu")$value
	bf.exp[ii] <- integrate(integrad.ind,lower=0,upper=100,theta0=theta0.post[[ii]],theta=theta.post[[ii]],omega.post=omega.post,post=post,X=data$Xind,nl.i=nl[ii],anc=anc,K=K,TT=TT,factor.name=factor.name,ind.name="exp")$value
	bf.gender[ii] <- integrate(integrad.ind,lower=0,upper=100,theta0=theta0.post[[ii]],theta=theta.post[[ii]],omega.post=omega.post,post=post,X=data$Xind,nl.i=nl[ii],anc=anc,K=K,TT=TT,factor.name=factor.name,ind.name="gender")$value
}
